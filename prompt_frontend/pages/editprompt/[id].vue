...
let title = ref(prompt.value.title)
let description = ref(prompt.value.description)
let position_salary = ref(prompt.value.position_salary)
let position_location = ref(prompt.value.position_location)
let company_name = ref(prompt.value.company_name)
let company_location = ref(prompt.value.company_location)
let company_email = ref(prompt.value.company_email)
let errors = ref([])
...
async function submitForm() {
    ...
    if (errors.value.length == 0) {
        await $fetch('http://127.0.0.1:8000/api/v1/prompts/' + route.params.id + '/edit/', {
            method: 'PUT',
            headers: {
...
<template>
    <div class="py-10 px-6">
        <h1 class="mb-6 text-2xl">Edit prompt</h1>
